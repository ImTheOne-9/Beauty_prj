...
import BeautyAppliedProducts from '@/features/beauty-try-on/presentation/components/BeautyAppliedProducts'
import BeautyProductGrid from '@/features/beauty-try-on/presentation/components/BeautyProductGrid'
import BeautyProductTabs from '@/features/beauty-try-on/presentation/components/BeautyProductTabs'
import BeautyVirtualMirror from '@/features/beauty-try-on/presentation/components/BeautyVirtualMirror'
import { PatternPickerModal } from '@/features/ai-scan/presentation/components/PatternPickerModal'
import { usePatternCatalog } from '@/features/ai-scan/presentation/hooks/usePatternCatalog'
import { getPatternColorCount, hasPatternCatalog } from '@/features/ai-scan/lib/makeup-patterns'
import { useFaceValidation } from '@/features/ai-scan/presentation/hooks/useFaceValidation'
import type { MakeupVtoTaskStatus } from '@/core/entities'
import {
...
export default function BeautyTryOnPage() {
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter((product) => product.categoryId === activeTab)
  }, [activeTab, products])
      ...
      current.some((item) => item.variantId === resolvedVariantId)
        ...
        : [
            ...current.filter((item) => {
              ...
              return currentProduct?.categoryId !== selectedProduct?.categoryId
            }),
            {
              productId,
  ...
  const getDefaultColorVariantIds = (
    ...
    const productVariants = variants.filter(
      (variant) => variant.productId === productId && variant.isActive && variant.colorHex?.trim(),
    )
    const orderedIds = [
      ...preferredVariantIds,
    ...
    mutationFn: async () => {
      ...
      const missingColors = visibleAppliedProducts.find((selection) => {
        ...
        const selectedColorCount = (selection.colorVariantIds ?? [selection.variantId]).filter((variantId) => {
          ...
          return Boolean(variant?.colorHex?.trim())
        }).length
