...
import type {
  ...
  ProductVariant as AdminProductVariantRecord,
  MakeupCatalogItem as MakeupCatalogRow,
} from '@/core/entities'
import {
...
function getVariantSwatchStyle(variant?: AdminProductVariantRecord | null) {
  ...
  if (isDualColorTexture(variant.texture)) {
    const shimmerColor = variant.shimmerColor ?? '#FFFFFF'
    return {
      background: `linear-gradient(135deg, ${variant.colorHex} 0 50%, ${shimmerColor} 50% 100%)`,
    }
  }

  return { backgroundColor: variant.colorHex }
}
...
export default function BeautyAppliedProducts({
  ...
  const appliedProductDetails = appliedProducts.map((selection) => {
    ...
    const productVariants = variants.filter(
      (item) => item.productId === selection.productId && item.isActive && item.colorHex?.trim(),
    )
    ...
    const colorSlots = Array.from({ length: requiredColorCount }).map((_, index) => {
      ...
      return {
        ...
        color: selectedVariant?.colorHex ?? null,
        label: selectedVariant?.name ?? selectedVariant?.colorHex ?? 'Not selected',
      }
    })
    return {
      ...
      imageUrl: variant?.imageUrl || getProductImage(product),
      color: colorSlots[0]?.color ?? variant?.colorHex ?? null,
                                ...
                                {product.productVariants.map((variant, variantIndex) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      aria-label={`Use ${variant.name ?? variant.colorHex} for color ${slot.index + 1}`}
                                      title={`${variant.name ?? variant.colorHex}${variant.texture ? ` - ${variant.texture}` : ''}`}
                                    />
                                  )
                                })}
                              </div>
