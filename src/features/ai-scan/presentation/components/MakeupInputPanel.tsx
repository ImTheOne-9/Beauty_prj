...
import { CategoryEffectEditor } from '@/features/ai-scan/presentation/components/CategoryEffectEditor'
...
import {
  ...
  PlaygroundSection,
  PlaygroundTabs,
} from '@/features/ai-scan/presentation/components/playground-ui'
...
type MakeupInputPanelProps = {
  ...
  categories?: Array<{ id: string; name: string; apiCategoryKey: string }>
            ...
            <div className="grid gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(categories ?? []).map((category) => {
                  const meta = MAKEUP_CATEGORY_META[category.apiCategoryKey]
                  const isOn = effects.find((e) => e.category === category.apiCategoryKey)?.enabled === true
                  return (
                    <PlaygroundCheckbox
                      key={category.apiCategoryKey}
                      checked={isOn}
                      label={meta?.label ?? category.name}
                      onChange={() => toggleCategory(category.apiCategoryKey)}
                    />
                  )
                })}
              </div>
